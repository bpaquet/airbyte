...
# The `X-RateLimit-*` headers on the connector's own responses are the only reading of the quota
# that is guaranteed to be the one its requests actually consume: `/rate_limit` and other external
# probes were observed reporting a different counter for the same installation. Shared (module-level)
# gate so this logs at most once per 30s total across every manifest stream and pagination call,
# mirroring `GithubStreamABC._log_rate_limit` on the still-Python streams.
_last_rate_limit_log_at: float = 0.0


def log_rate_limit(response: requests.Response, stream_name: str) -> None:
    global _last_rate_limit_log_at
    now = time.time()
    if now - _last_rate_limit_log_at < 30:
        return
    _last_rate_limit_log_at = now
    logger.info(
        "github_rate_limit: stream=%s remaining=%s limit=%s used=%s reset=%s",
        stream_name,
...
@dataclass
class RateLimitLoggingCursorPaginationStrategy(CursorPaginationStrategy):
    """`CursorPaginationStrategy` plus the same throttled `github_rate_limit` visibility the
    legacy Python streams log via `GithubStreamABC._log_rate_limit` - every manifest stream
    sharing `page_link_paginator` gets it for free. `stream_name` isn't reliably threaded into
    paginator construction by the CDK, so this falls back to the response's own path.
    """

    def __post_init__(self, parameters: Mapping[str, Any]) -> None:
        super().__post_init__(parameters)
        self._stream_name = (parameters or {}).get("stream_name")
    ...
    def next_page_token(
        ...
        log_rate_limit(response, self._stream_name or requests.utils.urlparse(response.url).path)
        return super().next_page_token(response, last_page_size, last_record, last_page_token_value)


@dataclass
class WorkflowRunsPaginationStrategy(RateLimitLoggingCursorPaginationStrategy):
