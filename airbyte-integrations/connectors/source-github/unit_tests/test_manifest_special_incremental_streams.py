...
def test_commits_two_syncs_legacy_scenario(rate_limit_mock_response, requests_mock):
    ...
    requests_mock.get(
        ...
        json=data[2:5],
        headers={"Link": f'<{commits}?per_page=100&after=cursor2&since=2022-02-02T10%3A10%3A06Z&sha=branch>; rel="next"'},
    )
    requests_mock.get(f"{commits}?per_page=100&after=cursor2&since=2022-02-02T10%3A10%3A06Z&sha=branch", json=data[5:7])
...
def test_workflow_runs_stop_paging_at_the_first_run_older_than_the_window(rate_limit_mock_response, requests_mock):
        ...
        [
            {
                ...
                "headers": {"Link": f'<{runs}?after=cursor2>; rel="next"'},
            },
            {
                "json": {"total_count": 3, "workflow_runs": [_run(1, "2021-11-01T00:00:00Z", _AFTER_START)]},
                "headers": {"Link": f'<{runs}?after=cursor3>; rel="next"'},
            },
    ...
    assert [request.qs.get("after") for request in requests_mock.request_history if request.path.endswith("/actions/runs")] == [
        None,
        ["cursor2"],
    ]
