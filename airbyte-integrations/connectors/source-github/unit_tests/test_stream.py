...
def test_next_page_token_prefers_after_cursor_over_page(requests_mock):
    ...
    cutoff while `after` does not. Every concrete REST stream that used to exercise this base
    class method migrated to the manifest in Steps 3-8; `ProbeStream` (see `utils.py`) is the
    generic test double that now stands in for them."""
    repository_args = {
        "repositories": ["organization/repository"],
        "page_size_for_large_streams": 100,
    }
    stream = ProbeStream(**repository_args)

    requests_mock.get(
        "https://api.github.com/repos/organization/repository/probe_stream",
        [
            {
                "json": [{"id": 1}],
                "headers": {
                    ...
                    "Link": '<https://api.github.com/repos/organization/repository/probe_stream?page=2&after=cursor2>; rel="next"'
                },
            },
            {"json": [{"id": 2}]},
        ],
    ...
    listings = requests_mock.request_history
